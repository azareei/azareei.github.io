...
description: Bayesian Machine Learning
img: /assets/research/bayes3.png
importance: 6
---
...
&=\max_{\theta} \sum_{i=1}^N \log \sum_{c=1}^2q(z_i=c) \frac{p(x_i,z_i=c\mid \theta)}{q(z_i=c)}) \\
&\geq \max_{\theta} \sum_{i=1}^N \sum_{c=1}^2 q(z_i=c) \log \frac{p(x_i,z_i=c\mid \theta)}{q(z_i=c)})
\end{align*}
$$
